  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        leading: IconButton(
          onPressed: () {
            print(label.payload.toString());
            Get.back();
          },
          icon: Icon(Icons.arrow_back_ios,color: Colors.black,),
        ),
        title: 
        Text(label.payload.toString().split('|')[0],style: TextStyle(color: Colors.black)),
        centerTitle: true,
      ),
      body: Center(
        child: Container(
          ...
          child: Center(
            child: Text(
              label.payload.toString().split('|')[1],style: TextStyle(color: Get.isDarkMode?Colors.black:Colors.white,fontSize: 30),
            ),
          ),
        ),
      ),
